(function($) {
    ...
    $document.ready(function() {
        ...
        if ($desctopMenu && $mobileMenuToggle) {
            ...
            $mobilepMenu.find('ul').empty().append(ttDesktopMenu);
...
if (plugins.galleryMoreLink.length) {
  ...
  $galleryMoreLink.on('click', function(e) {
    e.preventDefault();
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        // Append new items directly to the main gallery
        ...
        // Count loaded items and toggle Load More button
        var length = $('#gallery .gallery-item').length;
        var total_post = parseInt($('#ga_total_tes').val(), 10);

        if (length < total_post) {
      ...
      error: function(jqXHR, textStatus, errorThrown) {
        // ✅ use the loader you already defined
        $gallerymoreLoader.html(jqXHR.status + " :: " + textStatus + " :: " + errorThrown);
      }
    });
  });
}
